class Guidedetail < ActiveRecord::Base
  belongs_to :guide
  ...
  validates :NroLinDet, length:{maximum: 4, in=> 1..60}, presence: true
	...
	validates :QtyItem, length:{maximum: 18}, presence: true
	validates :SubQty, length:{maximum: 18}
	validates :SubCod, length:{maximum: 35}
	validates :FchElabor, length:{maximum: 10}
	validates :FchVencim, length:{maximum: 10}
